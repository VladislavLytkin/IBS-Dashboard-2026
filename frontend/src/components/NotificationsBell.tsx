...
const TYPE_DOT: Record<NotificationType, string> = {
  ...
  attendance: 'var(--orange)', system: 'var(--text-soft)',
}
      ...
      {open && (
        <div className="notif__panel">
          ...
          <div className="notif__list">
            ...
            {items.map((n) => (
              <div key={n.id} className={`notif__item${n.read ? '' : ' is-unread'}`} onClick={() => !n.read && markOne(n.id)}>
                ...
                <div>
                  ...
                  <div className="notif__time">{new Date(n.createdAt).toLocaleString('ru-RU')}</div>
                </div>
              </div>
            ))}
          </div>
