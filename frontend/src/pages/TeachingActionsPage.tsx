...
function AbsenceForm({ students, onCreated }: { students: { id: string; fullName: string; classId: string }[]; onCreated: () => void }) {
  const [form, setForm] = useState({ studentId: '', date: new Date().toISOString().slice(0, 10), lesson: '1', subject: 'Математика', type: 'excused' as const, reasonOrComment: 'Справка от врача' })
  ...
  return (
    <Card title="Проставить пропуск">
      <form className="form-grid" onSubmit={submit}>
        ...
        <label className="setting-field"><span className="field__label">Предмет</span><input className="input" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /></label>
...
function DebtForm({ students, onCreated }: { students: { id: string; fullName: string; classId: string }[]; onCreated: () => void }) {
  const [form, setForm] = useState({ studentId: '', subject: 'Математика', topic: '', reason: '', dueDate: new Date().toISOString().slice(0, 10), comment: '' })
  const submit = async (e: FormEvent) => {
    e.preventDefault()
...
function OfficeCallForm({ students, onCreated }: { students: { id: string; fullName: string; classId: string }[]; onCreated: () => void }) {
  ...
  const [form, setForm] = useState({ dateTime: '', room: '', reason: '' })
  const recipient = useMemo(() => {
    const student = students.find((s) => s.id === studentId)
    return (recipients.data ?? []).find((r) => r.fullName === student?.fullName)
  }, [recipients.data, studentId, students])
  const submit = async (e: FormEvent) => {
    e.preventDefault()
    if (!recipient) return
    await workflowService.sendMessage({ toUserId: recipient.id, type: 'office_call', title: 'Вызов в кабинет', text: `${form.reason}. Дата и время: ${form.dateTime}. Место: ${form.room}.`, meta: form })
    onCreated()
  }
  return (
    <Card title="Вызвать ученика">
      <form className="form-grid" onSubmit={submit}>
        ...
        <label className="setting-field"><span className="field__label">Причина вызова</span><input className="input" required value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} /></label>
        <button className="btn-primary" type="submit" disabled={!recipient}>Отправить вызов</button>
      </form>
    </Card>
  )
}
